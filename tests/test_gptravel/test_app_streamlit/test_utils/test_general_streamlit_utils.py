...
from gptravel.prototype.utils import is_valid_location, is_valid_openai_key, is_departure_before_return, get_score_map
...
def test_get_score_map_on_travel_plan(
    ...
    assert set(score_map['Activities Variety'].keys()) == {'score_value', 'score_weight',
    ...
    assert set(score_map['Activities Variety']['activities_distribution'].keys()) == set(labels_activities)
    for key in score_map['Activities Variety']['activities_distribution'].keys():
        assert isinstance(score_map['Activities Variety']['activities_distribution'][key], float)
    ...
    for key in score_map['Activities Variety']['labeled_activities'].keys():
        assert isinstance(score_map['Activities Variety']['labeled_activities'][key], dict)
        assert set(score_map['Activities Variety']['labeled_activities'][key].keys()) == set(labels_activities)
        assert all(isinstance(score_map['Activities Variety']['labeled_activities'][key][label], float) for label in
                   labels_activities)
        assert all(
            0 <= score_map['Activities Variety']['labeled_activities'][key][label] <= 1 for label in labels_activities)
